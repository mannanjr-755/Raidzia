...
async function confirmBooking(bookingId, userId) {
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (revenueAccount && cashAccount && (0, auth_1.toNum)(booking.downPayment) > 0) {
...
async function stockOut(itemId, quantity, projectId, notes) {
    return prisma_1.prisma.$transaction(async (tx) => {
        const item = await tx.inventoryItem.findUnique({ where: { id: itemId } });
        if (!item)
            throw new Error('Item not found');
        if ((0, auth_1.toNum)(item.quantity) < quantity)
        ...
        if (updated && (0, auth_1.toNum)(updated.quantity) <= (0, auth_1.toNum)(updated.minStock)) {
            ...
            for (const admin of admins) {
                await tx.notification.create({
                    data: {
                        ...
                        message: `${item.name} is below minimum stock (${(0, auth_1.toNum)(updated.quantity)} ${item.unit} remaining)`,
                        type: 'LOW_STOCK',
