...
router.get('/', auth_1.authenticate, (0, auth_1.authorize)('land:read'), async (req, res) => {
    ...
    const where = {
        deletedAt: null,
        ...(search ? { OR: [{ title: { contains: search, mode: 'insensitive' } }, { landId: { contains: search, mode: 'insensitive' } }] } : {}),
    };
...
router.put('/:id', auth_1.authenticate, (0, auth_1.authorize)('land:write'), async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ success: false, error: parsed.error.errors[0]?.message });
    try {
        const land = await prisma_1.prisma.landParcel.update({ where: { id }, data: parsed.data });
        res.json({ success: true, data: land });
    }
    catch (error) {
...
router.delete('/:id', auth_1.authenticate, (0, auth_1.authorize)('land:delete'), async (req, res) => {
    const id = param(req.params.id);
    try {
        await prisma_1.prisma.landParcel.update({ where: { id }, data: { deletedAt: new Date() } });
