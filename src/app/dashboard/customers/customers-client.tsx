...
export function CustomersPageClient() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await apiFetch<PaginatedResponse<Customer>>(`/api/customers?${qs}`);
  ...
  const handleExport = (format: 'excel' | 'pdf') => {
    window.open(`/api/export?entity=customers&format=${format}`, '_blank');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage customer records and balances</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input className="pl-10" placeholder="Search customers..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
        </div>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <Input placeholder="Customer Name *" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} disabled={saving} />
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will soft-delete the customer. This action can be reversed by an administrator.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
